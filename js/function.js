...
//Ajax, recupere la totalité de restaurant.json
...
function yelpApi() {
    $.post("yelp.php", { lat: loc.latitude, long: loc.longitude }).done(function(donnees) {
        ...
        for (var i = 0; i < dYelp.businesses.length; i++) {
            var restoYelp = {
                ...
                streetView: dYelp.businesses[i].image_url
            };
            data.push(restoYelp);
        }
        initMap(data);
...
function displayComments(id) {
    ...
    for (var l = 0; l < data[id].ratings.length; l++) {
        ...
        //Nombre d'étoiles pour chauque commentaire 
...
//insere les données du formuaire pour la création d'un nouveau restaurant
function newResto() {
